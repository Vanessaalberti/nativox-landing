...
**La prueba en la nube de la portada:** subtítulos en tiempo real, hasta 15 s por prueba. La portada usa el mismo flujo que la sesión en vivo de la aplicación (cortes en pausas de ~1,5 a 2 s, contexto de audio y de texto, filtro de alucinaciones) y manda cada fragmento (WAV) a un Worker chico (`servidor/`) que llama a Whisper large-v3 turbo en Workers AI con la cuenta de Vanessa. Hay dos pasadas: una provisoria cada 1,5 s, casi invisible, y la confirmada al cortar la frase. Los usos se cuentan en el servidor, en Durable Objects, **en segundos de audio facturado** (la pasada provisoria vuelve a mandar lo que se viene diciendo, así que cada segundo hablado factura ~2,5 a 4): 60 s por prueba, 3 pruebas por dispositivo (cookie que pone el servidor) cada 24 h, 12 por IP (por si se borra la cookie) y 12.000 s por día para todo el sitio (~200 min, lo que cubre la cuota diaria gratuita de Workers AI: unas 200 pruebas; hay que confirmarlo en la cuenta). Si Whisper falla (se reintenta una vez), ese audio no cuenta. Si se agotan, la portada ofrece "Probar" en la computadora.

...
- Comparación cargada con lo medido con mediciones propias (guion de prueba de 12 frases, 56 términos): WER, términos bien y retraso de las combinaciones que se probaron, cada uno con su fuente; lo que falta dice "a medir". La página ya no dice "charlas reales": todavía no se probó con charlas de Nerdearla.
- "Probar": elegir traductor (Bergamot o TranslateGemma 4B, también si la evaluación no lo recomienda), 4 pruebas por día con el micrófono de hasta 15 s (contadas en el servidor y aparte de las de la portada), vista a todo el ancho y botón "Test" en el menú. "Evaluar mi computadora" revisa el equipo (placa, RAM, núcleos, memoria máxima) y mide la potencia con un benchmark de WebGPU: no descarga modelos, no usa el micrófono ni audios.
- Sin `Referer` en todo el sitio (`Referrer-Policy: no-referrer`): Hugging Face responde 404 sin encabezados CORS a los pedidos con `Referer` de un sitio `*.workers.dev`, y por eso fallaba la descarga de los modelos ("Failed to fetch") en la versión publicada.
- La evaluación recomienda la versión de Whisper y el nivel de velocidad (con los motivos, en tres idiomas). La barra de nivel (Ahorro, Equilibrado, Rápido, Máximo) queda elegida sola y se puede cambiar.
